    ...
    if (selectedTab.id === 1) {
      return (
        ...
        >
          ...
          <span style={{ ...utils.rowCenter({ gap: 10 }), marginBottom: 6 }}>
            <svg.VideoSvg />
            <text.T16>{course.duration}</text.T16>
          </span>
          <span style={{ ...utils.rowCenter({ gap: 10 }), marginBottom: 6 }}>
            <svg.DownloadSvg />
            <text.T16>{course.category}</text.T16>
          </span>
          ...
          <components.Button
            title="Contact Us"
            onClick={() => {
              window.location.href = 'tel:+8801234567890'; // replace with your actual phone number
            }}
          />
        </div>
      );
